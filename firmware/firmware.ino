...
class ServerHandler : public BLEServerCallbacks
{
  ...
  {
    connected = false;
    Serial.println("blue tooth dis-connected");
...
bool take_photo() {
  ...
  Serial.println("one photo captured");
...
void handlePhotoControl(int8_t controlValue)
{
  ...
  else if (controlValue >= 1 && controlValue <= 300)
  {
    // Start taking photos at specified interval
    captureInterval = (controlValue / 1) * 1000; // Round to nearest 5 seconds and convert to milliseconds
    isCapturingPhotos = true;
...
void loop() {
  ...
  //  commandcam = Serial.read();
  //  Serial.printf("Cam status refreshed as %i \n",commandcam);
  //}
  if (Serial.available() > 0) {
    commandcam = Serial.parseInt();//-128 ~ 128
    // 检查是否成功读取到整数
      Serial.printf("Cam status refreshed as %i \n",commandcam);
      handlePhotoControl(commandcam);
  }
  // Read from mic
