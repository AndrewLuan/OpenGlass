...
async def play_audio_async_pygame(matches_results, mapping, audio_folder, control_queue):
    ...
    async with BleakClient(DEVICE_ADDRESS) as client:
        ...
        async def callback(sender, data):
            ...
            if frame_number == 0xFFFF:
                ...
                subject, pre_image_hist = analyze(image_data, pre_image_hist, pre_subject)
                if subject == pre_subject:
                    await play_audio_async_pygame(subject, mapping, audio_folder, control_queue)
                pre_subject = subject
...
def main():
    ...
    # 在单独的线程中运行 audio_control
    threading.Thread(target=audio_control, args=(control_queue,)).start()
